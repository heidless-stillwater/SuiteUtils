...
This plan details the synchronization and consolidation of all **`VideoSystem`** prompts, documents, and physical storage assets from their remote source projects into the local target Firebase project **`stillwater-sovereign-02`**, specifically mapping them to the target account **`heidlessemail21@gmail.com`** (`bc4g8h0VlgepUhg9nvjSMIsj2bx2`).
...
> - **Project Assets** (e.g. `projects/K3dLgayM7gnTnZpDXs7A/...`): To ensure references align correctly in the app's database, these will be copied to `projects/K3dLgayM7gnTnZpDXs7A/...` on the local bucket `stillwater-sovereign-02.firebasestorage.app` and marked public.
...
1. **Initializes Multiple Source Connections**:
   ...
   - Target (Local GCS & Firestore): `stillwater-sovereign-02.firebasestorage.app` (using `./suite-admin-sovereign.json`).
...
3. **Synchronizes Media Assets**:
   ...
   - If it is a **Project Asset** (starts with `projects/`):
     - Checks GCS target bucket `stillwater-sovereign-02` for the file.
   ...
   - If it is a **User Asset** (starts with `users/`):
     - Checks GCS target bucket `stillwater-sovereign-02` under path `users/bc4g8h0VlgepUhg9nvjSMIsj2bx2/images/[filename]`.
...
4. **Performs String Translation**:
   ...
   - Translates all remote bucket references in URLs to `stillwater-sovereign-02.firebasestorage.app`.
...
* **Final Database Audit**:
  Run a target check to confirm that `users/bc4g8h0VlgepUhg9nvjSMIsj2bx2/images` contains the remapped VideoSystem documents and that all media links resolve successfully under `stillwater-sovereign-02`.
