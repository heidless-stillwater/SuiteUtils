#!/bin/bash
# Stillwater Ollama Local Controller
# Unset leaked environment project variables to force loading from active config (.env)
unset GOOGLE_CLOUD_PROJECT
unset CLOUDSDK_CORE_PROJECT

APP_NAME="Ollama Service"
PORT=11434
LOG_FILE="/home/heidless/projects/SuiteUtils/logs/ollama.log"
...
case "$1" in
    start)
        ...
        if ! command -v ollama &> /dev/null; then
            echo "❌ Error: Ollama command not found in WSL2."
            echo "💡 Please run inside WSL2: curl -fsSL https://ollama.com/install.sh | sh"
            exit 1
        fi
