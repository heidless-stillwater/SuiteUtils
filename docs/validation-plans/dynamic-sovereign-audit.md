...
- [ ] **Verification**: [VAL-INF-001](http://localhost:5180/?validation=VAL-INF-001) - **VALIDATION_STRATEGY**: ENABLED — `VAL-INF-001`: After the fix, click Pull on any model in the catalog and verify the progress bar appears and increments (Ollama must *(Auto-registered from conversation-sensor)*


### [IDE Chat] Auto-detected from IDE chat
- [ ] **Verification**: [VAL-INF-002](http://localhost:5180/?validation=VAL-INF-002) - **VALIDATION_STRATEGY**: ENABLED — `VAL-INF-002`: After dev server restart, open browser DevTools → Network tab, click Pull, confirm a request to `/api/ollama/pull` re *(Auto-registered from conversation-sensor)*
