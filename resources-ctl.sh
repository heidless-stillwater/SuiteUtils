...
case "$1" in
    start)
        ...
        cd $APP_DIR
        nohup env PORT=${PORT} SERVICE_DATABASE_ID="promptresources-db-0" FIREBASE_DATABASE_ID="promptresources-db-0" NEXT_PUBLIC_FIREBASE_DATABASE_ID="promptresources-db-0" GOOGLE_APPLICATION_CREDENTIALS="/home/heidless/projects/PromptResources/service-account.json" npm run dev -- -p ${PORT} > $LOG_FILE 2>&1 &
